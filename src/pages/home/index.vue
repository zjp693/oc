<template>
  <view class="home-page">
    ...
    <view v-else class="tab-placeholder">
      <text class="placeholder-title">{{ currentTabTitle }}</text>
...
.desktop-page {
  min-height: 100vh;
  padding: calc(var(--status-bar-height) + 44rpx) 38rpx calc(112rpx + env(safe-area-inset-bottom));
...
.tab-placeholder {
  min-height: 100vh;
  padding: calc(var(--status-bar-height) + 44rpx) 40rpx calc(112rpx + env(safe-area-inset-bottom));
  box-sizing: border-box;
  display: flex;
  flex-direction: column;
  background-image: url('/static/login/page-bg.png');
